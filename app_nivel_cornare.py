...
nombre_estudiante = st.sidebar.text_input("Nombre del estudiante", "Julian Andres Bernal Hoyos")
codigo_estacion = st.sidebar.text_input("Código de estación", "28")
fecha_desde = st.sidebar.date_input("Desde", pd.to_datetime("2026-03-20")).strftime("%Y-%m-%d")
fecha_hasta = st.sidebar.date_input("Hasta", pd.to_datetime("2026-03-26")).strftime("%Y-%m-%d")
    ...
    if error:
        ...
        if not registros:
            ...
            if not coords_reales:
                st.caption("La API no trajo latitud/longitud de la estación — se muestra el punto de partida (Pascual Bravo). Ajusta `CANDIDATOS_LAT` / `CANDIDATOS_LON` si conoces el nombre re[...] )")
